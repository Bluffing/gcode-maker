namespace gcode_maker
{
    public class Coord
    {
        ...
        private bool IsBetween(double a, double b, double point) => (a < b && a <= point && point <= b) || (a > b && a >= point && point >= b);
        // public bool FitIn(Coord start, Coord end) => x >= start.x && x <= end.x && y >= start.y && y <= end.y;
        public bool FitIn(Coord start, Coord end) => IsBetween(start.x, end.x, x) && IsBetween(start.y, end.y, y);
